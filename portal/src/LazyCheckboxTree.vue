...
function activate(id: string): void {
  ...
  if (!node || node.disabled || node.loading || props.busy) return
  emit('toggle', id, !checked(id))
}

function activateFromPointer(id: string): void {
  focusKey(id)
  activate(id)
}
...
<template>
  ...
  >
    ...
    <button v-if="!isRoot && parentNode?.error" class="k-btn k-btn--ghost lazy-tree-more" role="treeitem" type="button" :data-tree-focus="`action:retry:${parentId}`" :tabindex="effectiveActiveId === `action:retry:${parentId}` ? 0 : -1" :aria-level="(parentNode.depth || 0) + 1" :disabled="parentNode.loading || busy" @focus="setActive(`action:retry:${parentId}`)" @click="runAction(`action:retry:${parentId}`, parentId!, () => emit('expand', parentId!))">Retry {{ parentNode.label }}</button>
    <button v-if="!isRoot && parentNode?.nextPageToken" class="k-btn k-btn--ghost lazy-tree-more" role="treeitem" type="button" :data-tree-focus="`action:more:${parentId}`" :tabindex="effectiveActiveId === `action:more:${parentId}` ? 0 : -1" :aria-level="(parentNode.depth || 0) + 1" :disabled="parentNode.loading || busy" @focus="setActive(`action:more:${parentId}`)" @click="runAction(`action:more:${parentId}`, parentId!, () => emit('load-more', parentId))">Load more in {{ parentNode.label }}</button>
    <p v-if="isRoot && rootError" class="lazy-tree-error" role="alert">{{ rootError }}</p>
    <button v-if="isRoot && rootError" class="k-btn k-btn--ghost lazy-tree-root-more" role="treeitem" type="button" data-tree-focus="action:root-retry" :tabindex="effectiveActiveId === 'action:root-retry' ? 0 : -1" aria-level="1" :disabled="rootLoading || busy" @focus="setActive('action:root-retry')" @click="runAction('action:root-retry', tree.roots[0] || 'action:root-retry', () => emit('load-more', undefined))">Retry</button>
    <p v-if="isRoot && rootLoading" class="import-loading" role="status"><LoaderCircle class="spin" :stroke-width="1.75" aria-hidden="true" /> Loading resources…</p>
    <button v-if="isRoot && rootNextPageToken" class="k-btn k-btn--ghost lazy-tree-root-more" role="treeitem" type="button" data-tree-focus="action:root-more" :tabindex="effectiveActiveId === 'action:root-more' ? 0 : -1" aria-level="1" :disabled="rootLoading || busy" @focus="setActive('action:root-more')" @click="runAction('action:root-more', tree.roots.at(-1) || 'action:root-more', () => emit('load-more', undefined))">Load more</button>
