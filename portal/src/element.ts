...
// DatabricksDashboardTileElement is the console's dashboard summary card. Same
// farosContext setter contract as the page element.
export class DatabricksDashboardTileElement extends HTMLElement {
  private _vueApp: VueApp | null = null
  private _state = reactive<{ ctx: FarosContext | null }>({ ctx: null })
  private _host: HTMLDivElement | null = null
  ...
  connectedCallback(): void {
    if (this._vueApp) return
    this._host = document.createElement('div')
    this._host.className = 'databricks-tile-host'
    this.appendChild(this._host)
    this._vueApp = createApp({
